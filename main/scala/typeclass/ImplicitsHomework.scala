...
object ImplicitsHomework {
  ...
  object SuperVipCollections4s {
    ...
    final class MutableBoundedCache[K: GetSizeScore, V: GetSizeScore](maxSizeScore: SizeScore) {
      ...
      import instances.linkedHashMapSizeScore
      import syntax._
    ...
    object instances {
      ...
      implicit val stringSizeScore: GetSizeScore[String] = (string: String) => 12 + string.length * 2

      implicit def listSizeScore[T: GetSizeScore]: GetSizeScore[List[T]] = (list: List[T]) => {
        list.foldLeft(12)((a, b) => a + b.sizeScore)
      }

      implicit def vectorSizeScore[T: GetSizeScore]: GetSizeScore[Vector[T]] = (vector: Vector[T]) => {
        vector.foldLeft(12)((a, b) => a + b.sizeScore)
      }

      implicit def arraySizeScore[T: GetSizeScore]: GetSizeScore[Array[T]] = (array: Array[T]) => {
        array.foldLeft(12)((a, b) => a + b.sizeScore)
      }

      implicit def mapSizeScore[K: GetSizeScore, V: GetSizeScore]: GetSizeScore[Map[K, V]] = (map: Map[K, V]) => {
        map.foldLeft(12)((a, b) => a + b._1.sizeScore + b._2.sizeScore)
      }

      implicit def linkedHashMapSizeScore[K: GetSizeScore, V: GetSizeScore]: GetSizeScore[mutable.LinkedHashMap[K, V]] = (map: mutable.LinkedHashMap[K, V]) => {
        map.foldLeft(0)((a, b) => a + b._1.sizeScore + b._2.sizeScore)
      }

      implicit def packedMultiMapGetSizeScore[T: GetSizeScore, S: GetSizeScore]: GetSizeScore[PackedMultiMap[T, S]] = (packedMultiMap: PackedMultiMap[T, S]) => {
        packedMultiMap.inner.foldLeft(12)((a, b) => a + b._1.sizeScore + b._2.sizeScore)
      }
    }
  ...
  object MyTwitter {
    ...
    implicit val twitGetSizeScore: GetSizeScore[Twit] = (twit: Twit) => {
      twit.id.sizeScore + twit.userId.sizeScore + twit.hashTags.sizeScore + twit.attributes.sizeScore + twit.fbiNotes.sizeScore
    }

    implicit val fbiNoteGetSizeScore: GetSizeScore[FbiNote] = (fbiNote: FbiNote) => {
    ...
    def createTwitCache(maxSizeScore: SizeScore): TwitCache = new TwitCache {
      private val cache = new MutableBoundedCache[Long, Twit](maxSizeScore: SizeScore)
